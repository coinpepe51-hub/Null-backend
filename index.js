...
// ============================================================
// EMAIL ACCOUNTS — from environment variables with fallback
// ============================================================
...
// ============================================================
// NODEMAILER SMTP — WITH YOUR CONFIG
// ============================================================
const transports = emailAccounts.map((account, index) => {
    console.log(`📧 Creating SMTP transport ${index + 1}: ${account.user}`)
...
async function verifyEmailAccounts() {
    ...
    for (let i = 0; i < transports.length; i++) {
        ...
        try {
            ...
            console.log(`✅ SMTP OK: ${account.user}`)
            workingCount++
        } catch (error) {
            console.error(`❌ SMTP FAILED: ${account.user}`)
            console.error({
    ...
    console.log(`\n📧 SMTP verification finished: ${workingCount} working, ${failedCount} failed\n`)
    return { workingCount, failedCount }
}

// ============================================================
// SEND EMAIL — WITH YOUR FUNCTION
// ============================================================
async function sendEmail({
    ...
    try {
        ...
        console.error(`❌ SMTP ERROR from ${account.user}`)
        console.error({
            message: error.message,
            code: error.code,
            command: error.command,
...
// ============================================================
// SUBMIT REPORT — USING YOUR sendEmail FUNCTION
// ============================================================
...
// Verify SMTP accounts before starting
verifyEmailAccounts().then(({ workingCount, failedCount }) => {
    if (workingCount === 0) {
        console.warn('⚠️ WARNING: No SMTP accounts are working! Emails will fail.')
    }
    
    app.listen(PORT, () => {
        console.log(`\n🔫 Null-Ban-Route backend running on port ${PORT}`)
        console.log(`📧 ${emailPool.length} email accounts (${workingCount} working, ${failedCount} failed)`)
    ...
    app.listen(PORT, () => {
        console.log(`\n🔫 Server started without SMTP verification`)
    })
})
