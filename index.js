...
// Download image from ImgBB URL with detailed logging
async function downloadEvidenceImage(url, outputPath, logFn) {
    try {
        if (logFn) await logFn(`   ⬇️ Downloading from: ${url.substring(0, 40)}...`)
        ...
        return new Promise((resolve, reject) => {
            writer.on('finish', () => {
                if (logFn) logFn(`   ✅ Downloaded: ${outputPath}`)
                resolve()
            })
            writer.on('error', (err) => {
...
// submit report via SMTP with image attachment
    ...
    try {
        ...
        for (let i = 0; i < count; i++) {
            ...
            try {
                ...
                if (!proxy) {
                    await updateLog(`⚠️ Report ${i+1}/${count}: no working proxies — using direct connection`)
                } else {
                    console.log(`[${jobId}] 🌐 Using proxy: ${proxy.host}:${proxy.port}`)
                }
                ...
                try { 
                    if (fs.existsSync(evidencePath)) {
                        ...
                        console.log(`[${jobId}] 🗑️ Deleted temp file: ${evidencePath}`)
                    }
                } catch (e) {
