...
// ============================================================
// SUBMIT REPORT — WITH HARD TIMEOUT
// ============================================================
async function submitReport(emailAccount, evidencePath, template, targetNumber) {
    ...
    try {
        ...
        // Try fallback endpoint with timeout
        ...
        try {
            ...
            const sendPromise = transporter.sendMail(mailOptionsFallback)
            const timeoutPromise = new Promise((_, reject) => {
                setTimeout(() => reject(new Error('SMTP timeout after 15 seconds')), 15000)
            })
            ...
            return { success: false, error: fallbackError.message, endpoint: fallback }
        }
    }
}
...
app.post('/api/execute', async (req, res) => {
    ...
    // Run in background, catch errors
