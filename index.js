...
// ============================================================
// EMAIL ACCOUNTS — from environment variables
// ============================================================
// Format: EMAIL_1=someone@gmail.com, EMAIL_APP_PASSWORD_1=xxxx
// EMAIL_2=another@gmail.com, EMAIL_APP_PASSWORD_2=xxxx
// etc.
...
// Fallback to hardcoded if env not set
...
console.log(`📧 Email pool: ${emailPool.length} accounts loaded from ${getEmailAccounts().length > 0 ? 'ENV' : 'fallback'}`)

// ============================================================
// NODEMAILER CONNECTION POOL
// ============================================================
const transports = emailAccounts.map(account =>
...
function getTransport() {
    const transport = transports[senderIndex]
    senderIndex = (senderIndex + 1) % transports.length
    return transport
}

async function sendEmail({ to, subject, text, attachments = [] }) {
    const transport = getTransport()
    const account = emailAccounts[transports.indexOf(transport)]
    ...
    try {
        ...
        console.error(`Email failed from ${account.user}:`, error.message)
        return {
            success: false,
            error: error.message,
            sender: account.user
...
// ============================================================
// TEMPLATES (condensed for space — full versions from previous)
// ============================================================
// [All templates from previous version go here — keeping them for brevity]
// I'll include the full templates in the final output
...
// ============================================================
// TEMPLATES — Perm (20) — abbreviated for length
// ============================================================
const permTemplates = [
    {
...
// Submit report using connection pool
async function submitReport(emailAccount, evidencePath, template, targetNumber) {
    ...
    // If first endpoint fails, try one more fallback
    if (!result.success) {
    ...
    try {
        ...
        for (let i = 0; i < count; i++) {
            ...
            try {
                ...
                if (result.success) {
                    usedEndpoints.push(result.endpoint || 'unknown')
                    await updateLog(`✅ Report ${i+1}/${count}: sent via ${result.sender || emailAccount.email}`)
                } else {
        ...
        await Job.updateOne(
            { jobId },
            { $set: { usedEndpoints: uniqueEndpoints } }
        )
...
app.listen(PORT, () => {
    ...
    console.log(`📧 ${emailPool.length} email accounts (connection pool: ${transports.length} transports)`)
