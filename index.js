...
// ============================================================
// BREVO SMTP — no domain needed
// ============================================================
const BREVO_SMTP_KEY = process.env.BREVO_SMTP_KEY || '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'

// Create a single Brevo SMTP transport
...
async function sendEmail({ to, subject, text, attachments = [] }) {
    try {
        ...
        const mailOptions = {
            from: 'no-reply@brevo.com', // Brevo's default sender
            to: to,
            subject: subject,
            text: text,
        ...
        const info = await brevoTransport.sendMail(mailOptions)
...
const permTemplates = [
    ...
    { subject: "DSA — Permanent Termination for Systemic Risk 2", body: `To the DSA Compliance Officer,\n\nTARGET_NUMBER poses systemic risk through disinformation campaigns.\n\nPermanent termination required.\n\nRegards,\n[redacted]` },
...
// SUBMIT REPORT
// ============================================================
async function submitReport(evidencePath, template, targetNumber) {
    ...
    try {
        ...
        for (let i = 0; i < count; i++) {
            ...
            try {
                ...
                await updateLog(`📤 Report ${i+1}/${count}: submitting via Brevo`)
                console.log(`[${jobId}] 📤 Sending report ${i+1}...`)

                const result = await submitReport(evidencePath, template, phone)
                ...
                if (result.success) {
                    usedEndpoints.push('brevo')
                    await updateLog(`✅ Report ${i+1}/${count}: sent via Brevo`)
                } else {
...
// Verify Brevo SMTP before starting
verifyBrevo().then(() => {
    app.listen(PORT, () => {
        ...
        console.log(`📫 Brevo SMTP: ${process.env.BREVO_SMTP_KEY ? '✅ configured' : '❌ not configured'}`)
